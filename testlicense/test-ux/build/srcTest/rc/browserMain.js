...
(function () {
  ...
  require.config({
    ...
    paths: {
      ...
      nmodule: '/module',
      'nmodule/test': 'build/karma/src',
      'nmodule/testTest': 'build/karma/srcTest',
  ...
  define('nmodule/test/rc/test.built.min');
  function testOnly(regex) {
    if (regex) {
      testGlobals.testOnly = regex;
    }
  }
  require(['niagara-test-server/karmaUtils', 'niagara-test-server/globals'], function (karmaUtils) {
    /*
    ...
    karmaUtils.setupAndRunSpecs({
      user: 'admin',
      pass: 'asdf1234',
      specs: ['nmodule/testTest/rc/spec/allSpecs']
    }, function (err) {
      if (err) {
        console.error(err);
      }
